...
- [X] Add a tmux-native shared-clipboard paste path at `prefix+p` via `bin/wslpaste`.
...
- [X] 2026-04-02 2.0 - `pretty_line_numbers` optimization (split normal/cursor caches, eagerly seed common widths, and add adjacent specs that lock in window-local state handling).
...
- [>] 2026-04-02 Re-test popup/help/LspInfo numberline corruption after the win-local state and stale-`statuscolumn` cleanup in `./lua/scripts/numberline.lua`.
    - [~] The old `0, 0, 0, 08, 09, 10...` symptom likely came from writing cursorline state through `vim.w` against the current window and leaving excluded windows with stale `statuscolumn` content.
...
- [X] 2026-04-02 Block Wrappin comment inheritance across code barriers and declaration-shaped selections such as `local function` / `if ... then`.
